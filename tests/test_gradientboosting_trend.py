#!/usr/bin/env python3
"""
Test GradientBoosting Trend Model
=================================
Test individuale per GradientBoosting_Trend usando dati reali dal backtest
"""
...
class GradientBoostingTrendTester:
    """Test specifico per GradientBoosting_Trend usando IDENTICAL struttura test_backtest"""
    
    def __init__(self):
        self.test_data_path = "./test_analyzer_data"
        self.symbol = "USTEC"
        self.model_name = "GradientBoosting_Trend"
        self.results = {}
    ...
    async def _perform_final_training(self):
        """Train ONLY GradientBoosting_Trend model - SPECIFIC to this test"""
        try:
            # Usa l'analyzer dell'unified system (AdvancedMarketAnalyzer)
            if self.unified_system and hasattr(self.unified_system, 'analyzer') and self.unified_system.analyzer:
                analyzer = self.unified_system.analyzer
                ...
                safe_print("🎯 Training ONLY GradientBoosting_Trend model...")
                
                # Get the asset analyzer
                if hasattr(analyzer, 'asset_analyzers') and asset_symbol in analyzer.asset_analyzers:
                    asset_analyzer = analyzer.asset_analyzers[asset_symbol]
                    
                    # Train only GradientBoosting_Trend model
                    if hasattr(asset_analyzer, '_retrain_algorithm'):
                        from src.Analyzer import ModelType
                        
                        # Get the algorithm from the competition
                        trend_competition = asset_analyzer.competitions.get(ModelType.TREND_ANALYSIS)
                        if trend_competition and 'GradientBoosting_Trend' in trend_competition.algorithms:
                            algorithm = trend_competition.algorithms['GradientBoosting_Trend']
                            
                            safe_print("🔄 Retraining GradientBoosting_Trend algorithm...")
                            ...
                            asset_analyzer._retrain_algorithm(ModelType.TREND_ANALYSIS, 'GradientBoosting_Trend', algorithm)
                            
                            safe_print("✅ GradientBoosting_Trend training completed")
                            ...
                            if hasattr(asset_analyzer, 'ml_models') and 'GradientBoosting_Trend' in asset_analyzer.ml_models:
                                model = asset_analyzer.ml_models['GradientBoosting_Trend']
                                safe_print(f"   Model Type: {model.__class__.__name__}")
                                
                                # Test a prediction
                                try:
                                    import numpy as np
                                    # GradientBoosting expects 2D array [n_samples, n_features]
                                    # 🚀 FIXED: Usa 38 features come _prepare_trend_dataset (SAME as RandomForest)
                                    test_input = np.random.randn(1, 38)  # 38 features come nel training!
                                    output = model.predict(test_input)
                                    safe_print(f"   Test Prediction: {output[0]:.6f}")
                                except Exception as e:
                                    safe_print(f"   ⚠️ Test prediction failed: {e}")
                            
                            return {"status": "success", "message": "GradientBoosting_Trend training completed"}
                        else:
                            safe_print("❌ GradientBoosting_Trend algorithm not found in competition")
                    ...
                    for i in range(min(1000, len(ticks) - processed_count)):
                        ...
                        if 'predictions' in analysis_result and analysis_result['predictions']:
                            ...
                            if predictions_count == 1:
                                ...
                                # Show trend prediction details
                                if 'trend_analysis' in analysis_result['predictions']:
                                    trend_pred = analysis_result['predictions']['trend_analysis']
                                    safe_print(f"   Trend Direction: {trend_pred.get('trend_direction', 'neutral')}")
                                    safe_print(f"   Trend Strength: {trend_pred.get('trend_strength', 0):.2%}")
                                    safe_print(f"   Confidence: {trend_pred.get('confidence', 0):.2%}")
    ...
    async def test_model(self):
        """Testa il modello GradientBoosting_Trend usando IDENTICAL struttura test_backtest"""
        safe_print("\n" + "="*80)
...
async def main():
    ...
    safe_print("🚀 Starting GradientBoosting Trend Model Test")
    safe_print("   Using IDENTICAL structure to test_backtest")
    safe_print("   📊 GradientBoosting has ANTI-OVERFITTING parameters!")
    ...
    tester = GradientBoostingTrendTester()
