#!/usr/bin/env python3
"""
Test Sentiment LSTM Model
=========================
Test individuale per Sentiment_LSTM usando dati reali dal backtest
"""
...
class SentimentLSTMTester:
    """Test specifico per Sentiment_LSTM usando IDENTICAL struttura test_backtest"""
    
    def __init__(self):
        self.test_data_path = "./test_analyzer_data"
        self.symbol = "USTEC"
        self.model_name = "Sentiment_LSTM"
        self.results = {}
    ...
    async def _perform_final_training(self):
        """Train ONLY Sentiment_LSTM model - SPECIFIC to this test"""
        try:
            # Usa l'analyzer dell'unified system (AdvancedMarketAnalyzer)
            if self.unified_system and hasattr(self.unified_system, 'analyzer') and self.unified_system.analyzer:
                analyzer = self.unified_system.analyzer
                ...
                safe_print("🎯 Training ONLY Sentiment_LSTM model...")
                
                # Get the asset analyzer
                if hasattr(analyzer, 'asset_analyzers') and asset_symbol in analyzer.asset_analyzers:
                    asset_analyzer = analyzer.asset_analyzers[asset_symbol]
                    
                    # Train only Sentiment_LSTM model
                    if hasattr(asset_analyzer, '_retrain_algorithm'):
                        from src.Analyzer import ModelType
                        
                        # Get the algorithm from the competition
                        bias_competition = asset_analyzer.competitions.get(ModelType.BIAS_DETECTION)
                        if bias_competition and 'Sentiment_LSTM' in bias_competition.algorithms:
                            algorithm = bias_competition.algorithms['Sentiment_LSTM']
                            
                            safe_print("🔄 Retraining Sentiment_LSTM algorithm...")
                            ...
                            asset_analyzer._retrain_algorithm(ModelType.BIAS_DETECTION, 'Sentiment_LSTM', algorithm)
                            
                            safe_print("✅ Sentiment_LSTM training completed")
                            ...
                            if hasattr(asset_analyzer, 'ml_models') and 'Sentiment_LSTM' in asset_analyzer.ml_models:
                                model = asset_analyzer.ml_models['Sentiment_LSTM']
                                ...
                                try:
                                    ...
                                    test_input = torch.randn(1, 50, 10)  # sequence of 50 timesteps with 10 features
                                    with torch.no_grad():
                                    ...
                                    safe_print(f"   Test Prediction Values: {output.numpy().flatten()[:3]}...")  # Show first 3
                                except Exception as e:
                                    safe_print(f"   ⚠️ Test prediction failed: {e}")
                            
                            return {"status": "success", "message": "Sentiment_LSTM training completed"}
                        else:
                            safe_print("❌ Sentiment_LSTM algorithm not found in competition")
                    ...
                    for i in range(min(1000, len(ticks) - processed_count)):
                        ...
                        if 'predictions' in analysis_result and analysis_result['predictions']:
                            ...
                            if predictions_count == 1:
                                ...
                                # Show bias detection details
                                if 'bias_detection' in analysis_result['predictions']:
                                    bias_pred = analysis_result['predictions']['bias_detection']
                                    safe_print(f"   Market Bias: {bias_pred.get('bias_direction', 'neutral')}")
                                    safe_print(f"   Bias Strength: {bias_pred.get('bias_strength', 0):.2%}")
                                    safe_print(f"   Confidence: {bias_pred.get('confidence', 0):.2%}")
    ...
    async def test_model(self):
        """Testa il modello Sentiment_LSTM usando IDENTICAL struttura test_backtest"""
        safe_print("\n" + "="*80)
...
async def main():
    ...
    safe_print("🚀 Starting Sentiment LSTM Model Test")
