...
def test_actual_analyzer():
    ...
    try:
        ...
        # Test the actual method through AssetAnalyzer
        print('\n🔧 Testing actual _prepare_volatility_dataset...')
        # Access the method directly from the main analyzer instance
        prepare_method = getattr(analyzer, '_prepare_volatility_dataset', None)
        if prepare_method:
            print(f'✅ Method _prepare_volatility_dataset found in analyzer instance')
            X, y = prepare_method(test_data)
        else:
            print('❌ Method _prepare_volatility_dataset not found')
