#!/usr/bin/env python3
"""
Test LSTM Sequences Model
=========================
Test individuale per LSTM_Sequences usando dati reali dal backtest
"""
...
class LSTMSequencesTester:
    """Test specifico per LSTM_Sequences usando IDENTICAL struttura test_backtest"""
    
    def __init__(self):
        self.test_data_path = "./test_analyzer_data"
        self.symbol = "USTEC"
        self.model_name = "LSTM_Sequences"
        self.results = {}
    ...
    async def _perform_final_training(self):
        """Train ONLY LSTM_Sequences model - SPECIFIC to this test"""
        try:
            # Usa l'analyzer dell'unified system (AdvancedMarketAnalyzer)
            if self.unified_system and hasattr(self.unified_system, 'analyzer') and self.unified_system.analyzer:
                analyzer = self.unified_system.analyzer
                ...
                if hasattr(analyzer, 'asset_analyzers') and asset_symbol in analyzer.asset_analyzers:
                    ...
                    if hasattr(asset_analyzer, '_retrain_algorithm'):
                        ...
                        if pattern_competition and 'LSTM_Sequences' in pattern_competition.algorithms:
                            algorithm = pattern_competition.algorithms['LSTM_Sequences']
                            
                            safe_print("🔄 Retraining LSTM_Sequences algorithm...")
                            ...
                            asset_analyzer._retrain_algorithm(ModelType.PATTERN_RECOGNITION, 'LSTM_Sequences', algorithm)
                            
                            safe_print("✅ LSTM_Sequences training completed")
                            ...
                            if hasattr(asset_analyzer, 'ml_models') and 'LSTM_Sequences' in asset_analyzer.ml_models:
                                model = asset_analyzer.ml_models['LSTM_Sequences']
                            ...
                            return {"status": "success", "message": "LSTM_Sequences training completed"}
                        else:
                            safe_print("❌ LSTM_Sequences algorithm not found in competition")
                            ...
                            if predictions_count == 1:
                                ...
                                if 'pattern_recognition' in analysis_result['predictions']:
                                    ...
                                    if 'detected_patterns' in pattern_pred and pattern_pred['detected_patterns']:
                                        ...
                                        safe_print(f"   Sequence Pattern: {first_pattern.get('pattern', 'No pattern')}")
                                        ...
                                        safe_print(f"   Sequence Pattern: No patterns detected")
    ...
    async def test_model(self):
        """Testa il modello LSTM_Sequences usando IDENTICAL struttura test_backtest"""
        safe_print("\n" + "="*80)
...
async def main():
    ...
    safe_print("🚀 Starting LSTM Sequences Model Test")
