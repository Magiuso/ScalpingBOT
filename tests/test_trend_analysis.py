...
def test_actual_analyzer():
    ...
    try:
        ...
        # Returns sui dati reali (fix broadcasting)
        price_changes = np.diff(prices, prepend=prices[0])
        ...
        # Analyze targets
        if len(y) > 0:
            print(f'\n📊 Real Implementation Trend Analysis (REAL DATA):')
            ...
            print(f'   Trend std: {np.std(y):.8f} (threshold: 1e-6)')
            ...
            if is_degenerate:
                ...
                if np.all(y == 0):
                    ...
                    print('   Reason: Very low variance in trend targets')
                else:
                    print('   Reason: Too few unique values in targets')
                return False
            else:
                print('✅ SUCCESS: Real implementation generates valid trend targets from REAL DATA')
                return True
        else:
