...
def test_actual_analyzer():
    ...
    try:
        ...
        if hasattr(analyzer, 'rolling_trainer'):
            prepare_method = getattr(analyzer.rolling_trainer, '_prepare_momentum_dataset', None)
            if prepare_method:
                print(f'✅ Method _prepare_momentum_dataset found in rolling_trainer')
                X, y = prepare_method(test_data)
            else:
                print('❌ Method _prepare_momentum_dataset not found in rolling_trainer')
