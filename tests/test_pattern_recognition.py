...
def test_actual_analyzer():
    ...
    try:
        ...
        prepare_method = getattr(analyzer, '_prepare_pattern_dataset', None)
        if prepare_method:
            print(f'✅ Method _prepare_pattern_dataset found in analyzer instance')
            X, y = prepare_method(test_data)
        else:
            print('❌ Method _prepare_pattern_dataset not found')
