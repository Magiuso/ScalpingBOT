#!/usr/bin/env python3
"""
Test RandomForest Trend Model
=============================
Test individuale per RandomForest_Trend usando dati reali dal backtest
"""
...
class RandomForestTrendTester:
    """Test specifico per RandomForest_Trend usando IDENTICAL struttura test_backtest"""
    
    def __init__(self):
        self.test_data_path = "./test_analyzer_data"
        self.symbol = "USTEC"
        self.model_name = "RandomForest_Trend"
        self.results = {}
    ...
    async def _perform_final_training(self):
        """Train ONLY RandomForest_Trend model - SPECIFIC to this test"""
        try:
            # Usa l'analyzer dell'unified system (AdvancedMarketAnalyzer)
            if self.unified_system and hasattr(self.unified_system, 'analyzer') and self.unified_system.analyzer:
                analyzer = self.unified_system.analyzer
                ...
                if hasattr(analyzer, 'asset_analyzers') and asset_symbol in analyzer.asset_analyzers:
                    ...
                    if hasattr(asset_analyzer, '_retrain_algorithm'):
                        ...
                        if trend_competition and 'RandomForest_Trend' in trend_competition.algorithms:
                            algorithm = trend_competition.algorithms['RandomForest_Trend']
                            
                            safe_print("🔄 Retraining RandomForest_Trend algorithm...")
                            ...
                            asset_analyzer._retrain_algorithm(ModelType.TREND_ANALYSIS, 'RandomForest_Trend', algorithm)
                            
                            safe_print("✅ RandomForest_Trend training completed")
                            ...
                            if hasattr(asset_analyzer, 'ml_models') and 'RandomForest_Trend' in asset_analyzer.ml_models:
                                model = asset_analyzer.ml_models['RandomForest_Trend']
                                safe_print(f"   Model Type: {model.__class__.__name__}")
                                
                                # Test a prediction
                                try:
                                    import numpy as np
                                    # RandomForest expects 2D array [n_samples, n_features]
                                    # 🚀 FIXED: Usa 38 features come _prepare_trend_dataset
                            ...
                            return {"status": "success", "message": "RandomForest_Trend training completed"}
                        else:
                            safe_print("❌ RandomForest_Trend algorithm not found in competition")
    ...
    async def test_model(self):
        """Testa il modello RandomForest_Trend usando IDENTICAL struttura test_backtest"""
        safe_print("\\n" + "="*80)
...
async def main():
    ...
    safe_print("🚀 Starting RandomForest Trend Model Test")
    safe_print("   Using IDENTICAL structure to test_backtest")
    safe_print("   🔥 NO FILTERS: Training on REAL movements!")
    ...
    tester = RandomForestTrendTester()
