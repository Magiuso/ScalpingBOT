"""
Universal Encoding Fix - MIGRATED from utils/universal_encoding_fix.py  
...
class UniversalEncodingFixer:
    ...
    def __init__(self, force_utf8: bool = True, silent: bool = False):
        ...
        # 🔧 FIXED MEMORY LEAK: Use bounded deque for fixes tracking
        from collections import deque
        self.fixes_applied: deque = deque(maxlen=20)  # Max 20 fixes tracked
    
    def _get_required_env_var(self, var_name: str) -> str:
        """Helper method to handle missing environment variables - FAIL FAST"""
    ...
    def detect_encoding_issues(self) -> Dict[str, Any]:
        ...
        issues = {
            ...
            'pythonioencoding': os.environ['PYTHONIOENCODING'] if 'PYTHONIOENCODING' in os.environ else self._get_required_env_var('PYTHONIOENCODING'),
            'console_cp': None,
            'issues_found': []
        }
        ...
        console_cp = issues['console_cp'] if 'console_cp' in issues else ''
